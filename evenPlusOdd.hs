...
oddPlusOdd OneOdd n = evenPlusEven ZeroEven (oddPlusOne n)
...
oddTimesOdd (NextOdd n) m = m `oddPlusEven` (m `oddPlusOdd` (oddTimesOdd n m))

-- | Proves even * odd = even
evenTimesOdd :: Even n -> Odd m -> Even (Mult n m)
evenTimesOdd ZeroEven m = ZeroEven
evenTimesOdd (NextEven n) m = m `oddPlusOdd` (m `oddPlusEven` (evenTimesOdd n m))

-- | Proves odd * even = even
oddTimesEven :: Odd n -> Even m -> Even (Mult n m)
oddTimesEven OneOdd m = m `evenPlusEven` ZeroEven
